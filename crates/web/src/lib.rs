...
pub async fn start_server(file: PathBuf, port: u16) -> Result<()> {
    ...
    println!("Menjalankan server web IPL pada http://{}", addr);
...
async fn handle_request(file: PathBuf) -> Html<String> {
    let kode_asli = match fs::read_to_string(&file) {
        Ok(k) => k,
        Err(_) => return Html("<h1>Error: Gagal membaca file script IPL.</h1>".to_string()),
    };

    let kode_sumber = if file.to_string_lossy().ends_with(".ipl.html") {
