...
pub fn register(vm: &mut VM) {
        ...
        func: |ctx, args| {
            ...
            if let Value::String(k_idx) = &args[0] {
                ...
                if let Some(sid) = session_id_opt {
                    ...
                    if found {
                        if let Some(val) = ctx.get_heap_mut().get_kamus(k_idx).get(&key) {
                            return Ok(val.clone());
                        }
                    }
                }
    ...
    let set_expired_func = FungsiBawaanVM {
        ...
        func: |ctx, args| {
            if args.len() != 1 {
                return Err("Fungsi 'session.set_expired' membutuhkan 1 argumen: durasi_detik".to_string());
            }
            ...
            let cookie_str = format!("RPL_SESSIONID={}; Path=/; HttpOnly; Max-Age={}", session_id, durasi);
            ctx.get_heap_mut().web_state.cookies_to_set.push(cookie_str);
    ...
    module_dict.insert("set_expired".to_string(), Value::FungsiBawaan(set_expired_idx));
