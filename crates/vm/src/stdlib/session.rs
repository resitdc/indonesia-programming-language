    ...
    let set_func = FungsiBawaanVM {
        ...
        func: std::sync::Arc::new(
            move |ctx: &mut dyn VmContext, args: Vec<Value>| -> Result<Value, String> {
                if args.len() != 2 {
                    return Err(
                        "Fungsi 'session.set' membutuhkan 2 argumen: kunci, nilai".to_string()
                    );
                }
                if let Value::String(k_idx) = &args[0] {
                        ...
                        .get(&session_id)
                    {
                        *idx
                        ...
                        let new_kamus_idx =
                            ctx.get_heap_mut().alloc(HeapData::Kamus(HashMap::new()));
                        ctx.get_heap_mut()
                            .web_state
                            .sessions
                            .lock()
    ...
    let set_expired_func = FungsiBawaanVM {
        ...
        func: std::sync::Arc::new(
            move |ctx: &mut dyn VmContext, args: Vec<Value>| -> Result<Value, String> {
                if args.len() != 1 {
                    return Err(
                        "Fungsi 'session.set_expired' membutuhkan 1 argumen: durasi_detik"
                            .to_string(),
                    );
                }
