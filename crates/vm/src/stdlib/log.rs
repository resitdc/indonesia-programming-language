...
fn log_impl(ctx: &mut dyn VmContext, args: Vec<Value>, level: &str, color: &str) -> Result<Value, String> {
    if args.is_empty() {
        return Err(format!("Fungsi 'log.{}' membutuhkan setidaknya 1 argumen", level));
    }
