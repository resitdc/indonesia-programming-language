...
pub fn inisialisasi(cwd: &Path) -> Result<()> {
    ...
    let default_config = PaketConfig {
        nama: cwd.file_name().unwrap_or_default().to_string_lossy().to_string(),
        ..Default::default()
    };
...
pub async fn instal(paket: Option<String>) -> Result<()> {
    ...
    if let Some(p) = paket {
        ...
        let nama = if parts.len() > 1 {
            parts[1].split('/').last().unwrap_or(&p).to_string()
        } else {
            p.clone()
        };
    ...
    for (nama, url) in dependensi.iter() {
        download_and_extract_recursive(nama.clone(), url.clone(), rpl_modules_dir.clone(), &mut installed).await?;
    }
        ...
        if has_rpl_json {
            ...
            if let Ok(config) = serde_json::from_str::<PaketConfig>(&content) {
                for (dep_nama, dep_url) in config.dependensi {
                    download_and_extract_recursive(dep_nama, dep_url, modules_dir.clone(), installed).await?;
                }
            }
        }
