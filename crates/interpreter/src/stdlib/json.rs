...
pub fn register(env: &Rc<RefCell<Lingkungan>>) {
    ...
    module_env.borrow_mut().set("buat".to_string(), Objek::FungsiBawaan(|args| {
    ...
    module_env.borrow_mut().set("parse".to_string(), Objek::FungsiBawaan(|args| {
        if args.len() == 1 {
    ...
    env.borrow_mut().set("json".to_string(), Objek::Modul(module_env));
}
