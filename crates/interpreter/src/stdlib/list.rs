...
pub fn register(env: &Rc<RefCell<Lingkungan>>) {
    ...
    module_env.borrow_mut().set("hapus".to_string(), Objek::FungsiBawaan(|args| {
        if args.len() == 2 {
            if let (Objek::Array(arr), Objek::Angka(idx)) = (&args[0], &args[1]) {
    ...
    module_env.borrow_mut().set("ambil".to_string(), Objek::FungsiBawaan(|args| {
        if args.len() == 2 {
            if let (Objek::Array(arr), Objek::Angka(idx)) = (&args[0], &args[1]) {
    ...
    env.borrow_mut().set("list".to_string(), Objek::Modul(module_env));
}
