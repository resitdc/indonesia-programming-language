...
pub fn register(env: &Rc<RefCell<Lingkungan>>) {
    // fungsi: tampilkan(x)
    env.borrow_mut().set("tampilkan".to_string(), Objek::FungsiBawaan(|args| {
        for arg in args {
    ...
    // fungsi: baca(prompt)
    env.borrow_mut().set("baca".to_string(), Objek::FungsiBawaan(|args| {
    ...
    // fungsi: angka(x)
    env.borrow_mut().set("angka".to_string(), Objek::FungsiBawaan(|args| {
    ...
    // fungsi: teks(x)
    env.borrow_mut().set("teks".to_string(), Objek::FungsiBawaan(|args| {
        if let Some(arg) = args.first() {
    ...
    // fungsi: boolean(x)
    env.borrow_mut().set("boolean".to_string(), Objek::FungsiBawaan(|args| {
        if let Some(arg) = args.first() {
            match arg {
                Objek::String(s) => {
