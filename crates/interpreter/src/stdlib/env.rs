...
pub fn register(env: &Rc<RefCell<Lingkungan>>) {
    ...
    // env.get("KEY")
    env_map.insert("get".to_string(), Objek::FungsiBawaan(|args| {
        if let Some(Objek::String(kunci)) = args.get(0) {
    ...
    // env.set("KEY", "VALUE")
    env_map.insert("set".to_string(), Objek::FungsiBawaan(|args| {
        if args.len() >= 2 {
            if let (Objek::String(kunci), Objek::String(nilai)) = (&args[0], &args[1]) {
                unsafe {
    ...
    env_map.insert("load".to_string(), Objek::FungsiBawaan(|args| {
        let result = if args.is_empty() {
            dotenvy::dotenv()
        } else if let Some(Objek::String(path)) = args.get(0) {
