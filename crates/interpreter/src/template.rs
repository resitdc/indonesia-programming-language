...
pub fn preprocess_template_to_function(input: &str) -> String {
    ...
    while i < chars.len() {
        if !in_ipl {
            ...
            if i < chars.len() && chars[i] == '<' && i + 1 < chars.len() && chars[i+1] != '/' && chars[i+1] != '?' {
                ...
                if k < chars.len() && chars[k] == '>' {
                    if tag_name.ends_with(".rpl") || tag_name.ends_with(".rpl.html") {
                        ...
                        output.push_str(&format!("  _html = _html + web.render(\"{}\", data)\n", path));
                        i = k + 1;
                        continue;
                    }
                }
            ...
            while i < chars.len() {
                ...
                if i + 4 < chars.len() && chars[i] == '<' {
                    ...
                    let mut tag_name = String::new();
                    let mut temp_j = i + 1;
                    while temp_j < chars.len() && chars[temp_j] != '>' && chars[temp_j] != ' ' && chars[temp_j] != '/' {
